...
음식점 메뉴판이 아니면 반드시 아래와 같이 빈 JSON 배열만 반환한다.

[]

음식점 메뉴판이라면 메뉴명과 가격을 추출한다.

규칙

1. 음식, 식사, 반찬, 디저트 메뉴만 추출한다.
...
3. 메뉴마다 반드시 아래 두 개의 필드를 포함한다.
- name
- price

4. price 규칙
- 메뉴판에 가격이 보이면 반드시 추출한다.
- ₩9000
- 9,000원
- 9000원
- 9,000
- 9000
위와 같이 어떤 형태든 숫자만 추출한다.
...
7. 메뉴 이름은 메뉴판에 적힌 그대로 반환한다.

8. 메뉴를 임의로 생성하거나 수정하지 않는다.

9. 모든 메뉴를 반드시 JSON 객체로 반환한다.

10. JSON 배열만 출력한다.

11. 절대 설명을 출력하지 않는다.

12. 절대 ```json 코드블록을 사용하지 않는다.
